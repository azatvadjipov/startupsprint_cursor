...
- `GET /api/health` — проверяет наличие критичных ENV, доступность `DATA_DIR` и читабельность файловой БД.
- На фронте доступна страница `/health`, которая визуализирует результат проверки и позволяет быстро увидеть, что именно сломалось на проде.
